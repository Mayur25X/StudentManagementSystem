...
const server = http.createServer((req, res) => {
  const user_ip = req.headers['req.socket.remoteAddress'] || req.socket.remoteAddress;
...
const restrictAccess = (req, res, next) => {
  ...
  if (allowed_ips.includes(user_ip) || user_ip === '::1' || user_ip === '::ffff:127.0.0.1') {
    ...
    conn.query(quy,[Student_id,email],(err,result)=>{
        ...
        else
        {
            if(result.length>0)
            {
                ...
                conn.query(quy2,[Student_id,name,email,Branch,gender],(err,results)=>{
                    if(err){
                        throw err
                    }
                    else
